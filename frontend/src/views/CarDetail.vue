<template>
  <div>
    ...
    <!-- Car Parts Section -->
    <div class="max-w-4xl w-3/4 p-6 bg-grey rounded-lg shadow-xl">
      <h2 class="text-2xl font-bold mb-4 text-gray-800">Car Parts</h2>
      <div v-for="part in specificCarParts" :key="part.id" class="mb-4 p-4 bg-gray-100 rounded-lg shadow-md">
        <p class="text-lg text-gray-900 mb-2"><span class="font-semibold">Car Part:</span> {{ part.name }}</p>
        <p class="text-lg text-gray-900"><span class="font-semibold">Price:</span> {{ part.price }}</p>
      </div>
      <button
        @click="showAddCarPartForm = true"
        class="bg-blue-500 text-white px-6 py-3 rounded hover:bg-blue-700 transition duration-300"
      >
        Add Car Part
      </button>
      <AddCarPartForm
        v-if="showAddCarPartForm"
        @submit="handleAddCarPart"
        @cancel="showAddCarPartForm = false"
      />
    </div>
  </div>
</template>
...
import AddCarPartForm from '@/components/AddCarPartForm.vue'; // Adjust import based on your component path
import Carousel from '@/components/Carousel.vue';
import { ref, onMounted } from 'vue';
import { useRoute, useRouter } from 'vue-router';
import { useCarStore } from '@/stores/useCarStore';
import { useServiceHistoryStore } from '@/stores/useServiceHistoryStore';
import { useCarPartsStore } from '@/stores/useCarPartsStore'; // Adjust import based on your store path
...
const { specificCarServiceHistories, delete: deleteServiceHistory, store: addServiceHistory } = useServiceHistoryStore();
const { specificCarParts, addCarParts } = useCarPartsStore(); // Adjust based on your store methods
...
const showAddCarPartForm = ref(false);
...
onMounted(async () => {
  ...
  await serviceHistoryStore.specificCarServiceHistories(parseInt(carId as string));
  await specificCarParts(parseInt(carId as string));
});
...
const handleDeletionServiceHistory = async () => {
  ...
  try {
    if (serviceHistoryIdToDelete.value) {
      ...
      await specificCarServiceHistories(parseInt(carId as string));
    }
...
const handleAddServiceHistory = async (serviceId: number) => {
  ...
  try {
    ...
    await specificCarServiceHistories(parseInt(carId as string)); // Refresh service histories
  } catch (error) {
    console.error('Failed to add service history:', error);
  }
};

const handleAddCarPart = async (carPartId: number) => {
  const carId = parseInt(route.params.id as string, 10);

  try {
    // Process your form data or API call to add car part
    // Example: Call to store method in store
    const carIdRequest = {car_id: carId}
    await carPartsStore.addCarParts(carPartId, carIdRequest);
    console.log(carPartId)

    showAddCarPartForm.value = false;
    await carPartsStore.specificCarCarParts(carId); // Refresh car parts list
  } catch (error) {
    console.error('Failed to add car part:', error);
  }
};
</script>
